...
class SearchGamesUseCaseTest {
    ...
    fun invoke_returnsFlowFromRepository() = runTest {
        ...
        every { repository.searchGames("rpg") } returns flowOf(games)
    ...
    fun invoke_passesQueryToRepository() = runTest {
        every { repository.searchGames(any()) } returns flowOf(emptyList())

        useCase("shooter").collect { }

        io.mockk.verify { repository.searchGames("shooter") }
    }
